...
export class EpisodeTracking extends React.Component {
    ...
    render() {
        ...
        for (let show of this.state.shows) {
            rows.push(tr(
                ...
                td({className: 'status'}, this._getStatus(show.curr))
            ), tr(
    ...
    _getStatus(ep) {
        ...
        let watchedStyle = ep.en == epStatus.WATCHED ? 'active' : 'inactive';
        return div(
            ...
            span({className: watchedStyle + ' glyphicon glyphicon-eye-open'})
        );
    }
}
